...
def enriched_technical_signal(price: float, sma_20, sma_50, sma_200, rsi_14,
                              advanced: dict) -> dict:
    """Richer technical layer combining SMA + RSI + MACD + ADX + BB.

    Returns direction + strength + N votes. Returns 5-8 votes vs the 4-5 of
    the basic signal → higher signal coverage = higher confidence.
    """
    votes = []  # +1 bullish, -1 bearish, 0 neutral
    detail = {"sma": None, "rsi": None, "macd": None, "bb": None, "adx": None}

    # SMA votes
    if sma_20:    votes.append(+1 if price > sma_20 else -1)
    if sma_50:    votes.append(+1 if price > sma_50 else -1)
    if sma_200:   votes.append(+1 if price > sma_200 else -1)
    if sma_50 and sma_200:
        votes.append(+1 if sma_50 > sma_200 else -1)
        detail["sma"] = "Golden Cross" if sma_50 > sma_200 else "Death Cross"

    # RSI vote
    if rsi_14 is not None:
        if rsi_14 > 70:   votes.append(-1); detail["rsi"] = "overbought"
        elif rsi_14 < 30: votes.append(+1); detail["rsi"] = "oversold"
        elif rsi_14 > 55: votes.append(+1); detail["rsi"] = "bullish momentum"
        elif rsi_14 < 45: votes.append(-1); detail["rsi"] = "bearish momentum"
        else:              detail["rsi"] = "neutral"
    ...
    if bb_sig == "below_lower_band":
        votes.append(+1); detail["bb"] = "oversold (below lower band)"
    elif bb_sig == "above_upper_band":
        votes.append(-1); detail["bb"] = "overbought (above upper band)"

    # ADX vote (only weighs in if trend is strong)
    adx_data = advanced.get("adx") or {}
    if adx_data.get("adx") and adx_data["adx"] > 25:
        votes.append(+1 if adx_data["direction"] == "up" else -1)
        detail["adx"] = f"{adx_data['trend']} {adx_data['direction']}trend"
